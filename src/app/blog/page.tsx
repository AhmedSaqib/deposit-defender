  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <p className="text-zinc-600 text-sm mb-10">Guides and insights for resellers who want to understand their numbers.</p>
        ...
        <div className="space-y-4">
          {POSTS.map(post => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="block bg-white border border-zinc-200 hover:border-zinc-400 rounded-2xl p-6 transition-colors"
            >
              <p className="text-xs text-zinc-500 mb-2">{post.date} · {post.readingTime}</p>
              <h2 className="text-zinc-900 font-semibold text-lg mb-2 leading-snug">{post.title}</h2>
              <p className="text-zinc-600 text-sm leading-relaxed">{post.description}</p>
              <p className="text-emerald-600 text-sm mt-4">Read →</p>
            </Link>
          ))}
        </div>
      </main>
