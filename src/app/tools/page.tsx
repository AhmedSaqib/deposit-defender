  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <p className="text-zinc-600 text-sm mb-10">No account needed. Built for resellers who want to understand their numbers.</p>
        ...
        <div className="space-y-4">
          {tools.map(tool => (
            <Link
              key={tool.href}
              href={tool.href}
              className="flex items-start gap-5 bg-white border border-zinc-200 hover:border-zinc-400 rounded-2xl p-6 transition-colors group"
            >
              <span className="text-3xl mt-0.5">{tool.emoji}</span>
              <div>
                <h2 className="font-semibold text-zinc-900 group-hover:text-emerald-600 transition-colors mb-1">{tool.title}</h2>
        ...
        <div className="mt-12 bg-white border border-zinc-200 rounded-2xl p-6 text-center">
          <p className="text-sm font-medium text-zinc-900 mb-1">Want to track every sale automatically?</p>
          <p className="text-sm text-zinc-600 mb-5">MarginLog logs your profit, fees, and analytics across every platform — free to start.</p>
          <Link
            href="/signup"
