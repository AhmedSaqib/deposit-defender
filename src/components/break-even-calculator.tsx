    ...
    <div className="bg-white border border-zinc-200 rounded-2xl p-6 space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select
            ...
            className="w-full bg-white border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:border-emerald-500"
          >
            {PLATFORMS.map(p => (
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="0.00"
              className="w-full bg-white border border-zinc-300 rounded-lg pl-7 pr-3 py-2 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-emerald-500"
            />
          </div>
        </div>
        ...
        <div>
          ...
          {platform.slug === 'poshmark' && (
            <p className="text-xs text-zinc-500 mt-1">Poshmark provides a prepaid label to buyers — enter $0 unless you upgraded shipping.</p>
          )}
          {platform.slug === 'vinted' && (
            <p className="text-xs text-zinc-500 mt-1">Vinted buyer pays shipping — enter $0.</p>
          )}
        </div>
        ...
        <div className="border-t border-zinc-200 pt-5 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-zinc-100 rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-zinc-900">{fmt(breakEven)}</p>
              <p className="text-xs text-zinc-500 mt-1">Minimum to cover all costs</p>
            </div>
            {targetVal > 0 && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                <p className="text-xs text-zinc-600 mb-1">Price to hit {fmt(targetVal)} profit</p>
                <p className="text-2xl font-bold text-emerald-600">{fmt(targetPrice)}</p>
                <p className="text-xs text-zinc-500 mt-1">Including all costs and fees</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-zinc-50 rounded-xl p-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t border-zinc-300 pt-2 text-zinc-700 font-medium">
          ...
          <p className="text-xs text-zinc-500">
            ...
            {' '}<Link href={`/calculators/${platform.slug === 'facebook' ? 'facebook-marketplace' : platform.slug}`} className="text-emerald-600 hover:text-emerald-500">Open full {platform.name} calculator →</Link>
          </p>
        </div>
      )}
      ...
      {!hasInput && (
        <div className="text-center py-4">
          <p className="text-sm text-zinc-500">Enter what you paid above to see your break-even price.</p>
        </div>
      )}
    </div>
  )
